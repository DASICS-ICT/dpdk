	...
	for (i = 0; i < 4; ++i, ++txdp, ++pkts, ++txep) {
...
#ifdef RTE_ENABLE_DBCHECKER
		{
			dma_addr_t translated = dbchecker_alloc_mtdt(buf_dma_addr,
				(size_t)(*pkts)->buf_len, DMA_TO_DEVICE, DEV_ID, true);
			// printf("DBCHECKER: alloc mtdt for buf_dma_addr 0x%lx -> 0x%lx\n",
			// 	(unsigned long)buf_dma_addr, (unsigned long)translated);
...
next_desc:
		...
		if (!bulk_alloc) {
			...
			{
				dma_addr_t translated = dbchecker_alloc_mtdt((dma_addr_t)_dma,
					(size_t)nmb->buf_len, DMA_FROM_DEVICE, DEV_ID, false);
        // printf("DBCHECKER: alloc mtdt for dma_addr 0x%lx -> 0x%lx\n",
				// (unsigned long)_dma, (unsigned long)translated);
				if (translated != (dma_addr_t)-1) {
					_dma = translated;
					rxe->pkt_addr = translated;
				} else {
			 ...
			 const struct rte_eth_txconf *tx_conf)
{
    ...
    // printf("DBCHECKER: alloc mtdt for dma_addr 0x%lx -> 0x%lx\n",
		// 		(unsigned long)tz->iova, (unsigned long)translated);
		if (translated != (dma_addr_t)-1)
			 ...
			 struct rte_mempool *mp)
{
    ...
    // printf("DBCHECKER: alloc mtdt for dma_addr 0x%lx -> 0x%lx\n",
    //     (unsigned long)rz->iova, (unsigned long)translated);
		if (translated != (dma_addr_t)-1)
