...
static uint64_t vnic_tx_burst(struct vnic_rxtx_desc_queue *txq, uint64_t nb_pkts, uint64_t pkt_size) 
{
	...
	for (i = 0; i < nb_pkts; i++) {
		...
		#else
			tx_desc->buf = dbchecker_alloc_mtdt(orig_buf, pkt_size, DMA_TO_DEVICE, VNIC_DEV_ID);
...
static uint64_t vnic_tx_poc(struct vnic_rxtx_desc_queue *txq, enum vnic_desc_op op) 
{
	...
	for (i = 0; i < 1; i++) {
		...
		txq->tail = next_tail;
			...
			tx_desc->buf = orig_buf;
		#else
			tx_desc->buf = dbchecker_alloc_mtdt(orig_buf, 64, DMA_TO_DEVICE, op == VNIC_DESC_OP_DEV_SPOOF ? 0xf : VNIC_DEV_ID);
...
static uint64_t vnic_rx_burst(struct vnic_rxtx_desc_queue *rxq, uint64_t nb_pkts, uint64_t pkt_size) 
{
	...
	for (i = 0; i < nb_pkts; i++) {
		...
		#else
			rx_desc->buf = dbchecker_alloc_mtdt(orig_buf, pkt_size, DMA_FROM_DEVICE, VNIC_DEV_ID);
